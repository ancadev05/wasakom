                    ...
                    <form action="{{ url('/user-create') }}" method="post">
                        ...
                        <div class="mb-3">
                            <label class="form-label" for="level_akun_id">Level <span
                                    class="text-danger fw-bold">*</span></label>
                            <select class="form-select @error('level_akun_id') is-invalid @enderror" name="level_akun_id"
                                id="level_akun_id">
                                <option value="" selected>---</option>
                                @foreach ($level_akun as $item)
                                    <option value="{{ $item->id }}"
                                        {{ old('level_akun_id') == $item->id ? 'selected' : '' }}>
                                        {{ $item->level }}</option>
                                @endforeach
                            </select>
                            @error('level_akun_id')
                        ...
                        <div class="d-flex justify-content-end">
                            ...
                            <a href="{{ url('/user') }}" type="submit"
                                class="btn btn-sm btn-danger me-2 shadow-sm">Batal</a>
                        </div>
                    </form>
                </div>
            </div>
